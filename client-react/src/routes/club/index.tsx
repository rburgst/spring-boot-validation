...
import { ClubFilterBar } from '../../components/club-filter-bar'
...
export const ClubListPage: FC = () => {
  ...
  const onFilter = (newFilters: ClubFilter) => {
    navigate({
      to: clubsRoute.id,
      search: {
        ...search,
  ...
  return (
    <div>
      ...
      <ClubFilterBar
        onFilterUpdated={onFilter}
        clubName={search.filter?.clubName ?? undefined}
        managerEmail={search.filter?.managerEmail ?? undefined}
      />
      <DataTable
        lazy={true}
        value={clubs}
